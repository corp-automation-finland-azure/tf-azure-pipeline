...
if [[ "$cloneUrl" == "$SETTINGS_CLONESOURCETERRAFORMCODE" ]]; then
    ...
    git clone https://$gitToken@$SETTINGS_GITHUBURL/$targetGitOrganization/$targetDir.git
    ...
    git clone https://azdo:$gitToken@dev.azure.com/$targetGitOrganization/$targetProject/_git/$targetDir $targetDir
fi